...
import { ITableDefinition, IExtendedTreeNode, NodeType } from "../types/types";
import {
  createElement,
  createSVG,
  formatBytes,
...
const filesTable: ITableDefinition = {
  columns: [
    ...
    },
    {
      ...
      renderValue: (dataRow) =>
        dataRow.type === NodeType.Folder ? "" : formatBytes(dataRow.size),
    },
  ],
};
...
export class Table {
  ...
  updateSort = (sortKey: string | undefined) => {
    if (sortKey) {
  ...
  createTableHeader = (tableDefinition: ITableDefinition) => {
  ...
  createTableRow = (
    ...
    const cells = tableDefinition.columns.map((c) => {
      ...
      const link = createElement(
        "a",
        c.renderValue(node),
        { href: `#/${node.urlPath}`, class: !isFolder(node) ? "disabled" : "" },
        [],
        elements
      );
    ...
    return createElement("tr", "", {}, [...cells]) as HTMLTableRowElement;
  };

  // Create a table
  createTable = (tableDefinition: ITableDefinition) => {
    // Create a header
    ...
    const rows =
      tableDefinition.rows?.map((row) =>
        this.createTableRow(tableDefinition, row)
      ) ?? [];
    ...
    return createElement(
      "table",
      "",
      { id: "node_list_table", class: "table" },
      [tableHeader, ...rows]
    );
  };
}
